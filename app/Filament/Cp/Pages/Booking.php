...
//        }
    ...
    {
        return \App\Models\Booking::query()->where('user_id', '=', Auth::id());
    }
    ...
    {
        return [
            Action::make('downloadCertifcate')
                ->label(__('downloadCertifcate'))
...
//                    }
            ...
            })->visible(fn(\App\Models\Booking $record) => $record->status == 2),
            Action::make('cancel')
                ->label(__('cancel'))
                ->action('cancel')
                ...
                ->color('danger')
                ->hidden(fn(\App\Models\Booking $record) => $record->status === 3),
        ];
    }
