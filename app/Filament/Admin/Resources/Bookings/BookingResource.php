...
class BookingResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        ...
        return $table
            ->columns([
                TextColumn::make('workshop.title')->label(__('Workshop'))->searchable(),
                TextColumn::make('slot.name')->label(__('slot'))->searchable(),
...
//                TextColumn::make('answers'),
                ...
                TextColumn::make('rejection_message')->label(__('rejection_message')),
                TextColumn::make('status')->formatStateUsing(fn() => match ($state) {
                    0 => __('Waiting'),
            ...
            ->recordActions([
                ...
                Action::make('approve')
                    ...
                    ->schema([
                        Radio::make('type')->label(__('Message Type'))->options([
                            'default' => 'Default',
                            'custom' => 'Custom',
                        ])
                            ->reactive()
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Select::make('workshop_id')->options(Workshop::all()->pluck('title',
                    'id'))->searchable()
                    ->label(__('Workshop'))
                    ->reactive()
                    ->afterStateUpdated(fn(callable $set) => $set('slot_id', null))
