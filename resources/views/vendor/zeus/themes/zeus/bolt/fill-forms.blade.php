...
<div class="not-prose" style="{{ $colors }}">
    @if(!$inline)
        @if(!class_exists(\LaraZeus\BoltPro\BoltProServiceProvider::class) || (optional($zeusForm->options)['logo'] === null && optional($zeusForm->options)['cover'] === null))
            <x-slot name="header">
                <h2 class="text-white not-italic text-4xl normal">{{ $zeusForm->name ?? '' }}</h2>
                <p class="text-white font-normal not-italic my-2">{{ $zeusForm->description ?? '' }}</p>

                @if($zeusForm->start_date !== null)
                    <div class="text-white text-sm flex items-center">
                        @svg('heroicon-o-calendar','size-5 inline-flex mr-2')
